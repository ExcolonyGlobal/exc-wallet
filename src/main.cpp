...
unsigned int nStakeMinAge = 365 * 24 * 60 * 60; // 1 year as zero time weight
unsigned int nStakeMaxAge = 3 * 365 * 24 * 60 * 60; // 3 year as full weight
int64_t nTargetSpacing = 10 * 60;  // Time between reward:    600 Second
...
const string strMessageMagic = "excolonycoin Signed Message:\n";
...
// miner's coin base reward premine -- Premined:90.000.000
int64_t GetProofOfWorkReward(int64_t nFees)
{
        int64_t nSubsidy = 1 * COIN;
        
        if (pindexBest->nHeight+1 < 2)
            nSubsidy = 90000000 * COIN;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // excolonycoin: check proof-of-stake block signature
...
// excolonycoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "excolonycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "excolonycoin.com Block";
        CTransaction txNew;
        txNew.nTime = 1534470000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1534470000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2431498;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x355f5016b4588f429035f4d59fd0e319a9b874be656152b5e64f312e2ad0842d"));
        block.print();
